...
public class Indicators extends Module {
    ...
    private boolean shouldRender(Entity entity) {
        double d = (entity.posX - entity.lastTickPosX) * (mc.thePlayer.posX - entity.posX) + (entity.posY - entity.lastTickPosY) * (mc.thePlayer.posY + (double) mc.thePlayer.getEyeHeight() - entity.posY - (double) entity.height / 2.0) + (entity.posZ - entity.lastTickPosZ) * (mc.thePlayer.posZ - entity.posZ);
        if (d == 0.0) {
            return false;
        }
        if (d < 0.0) {
    ...
    public void onRender(Render2DEvent render2DEvent) {
        ...
        for (Entity entity : TeamUtil.getLoadedEntitiesSorted().stream().filter(this::shouldRender).collect(Collectors.toList())) {
            ...
            float yawBetween = RotationUtil.getYawBetween(RenderUtil.lerpDouble(mc.thePlayer.posX, mc.thePlayer.prevPosX, render2DEvent.getPartialTicks()), RenderUtil.lerpDouble(mc.thePlayer.posZ, mc.thePlayer.prevPosZ, render2DEvent.getPartialTicks()), RenderUtil.lerpDouble(entity.posX, entity.prevPosX, render2DEvent.getPartialTicks()), RenderUtil.lerpDouble(entity.posZ, entity.prevPosZ, render2DEvent.getPartialTicks()));
            ...
            String string = String.format("%dm", (int) mc.thePlayer.getDistanceToEntity(entity));
            GlStateManager.pushMatrix();
            GlStateManager.translate((offset + 0.0f) * x - (float) mc.fontRendererObj.getStringWidth(string) / 2.0f + 1.0f, (offset + 0.0f) * z + 1.0f, -100.0f);
            mc.fontRendererObj.drawStringWithShadow(string, 0.0f, 0.0f, ChatColors.GRAY.toAwtColor() & 0xFFFFFF | 0xBF000000);
            GlStateManager.popMatrix();
            GlStateManager.pushMatrix();
